...
| Manufactured attesters do not move an evaluation for someone who trusts none of them | **proven**, and now quantitative rather than only the zero-attack-edge case. Under the reference evaluator policy, standing is maximum flow from the evaluator's trust roots across the receipt graph under conservation, so what an adversary can capture is bounded by the capacity of the edges reaching it from parties the evaluator genuinely trusts. Measured flat from 10 to 10,000 adversary identities at fixed attack edges, and linear in attack edges; a fabricated corpus of any size captures exactly nothing. The flow computation is checked against a minimum cut enumerated independently in another language, with a negative control proving that check can fail |
| A relying party's total exposure is bounded, not just each decision's | **proven, and it was wrong first.** Per-subject standing summed across subjects is not a bound — it is the amplification. One attack edge of capacity c reaching N identities gives each of them standing c when evaluated alone, so extending value to each restores an N-fold exposure. They share an inflow and must be evaluated jointly |
| The trust loop completes with no network access | **proven** — but read it precisely, because it is routinely overstated into something false. *Verification* is host-free: a receipt, an equivocation proof and a standing computation all check offline, forever, from the bytes alone. *Pricing* is not. Adverse evidence bounds nobody unless it reaches a future evaluator, and a victim's receipt on the victim's laptop burns no one. Discovery is what hosts are for, so host plurality is a component of the security budget rather than an availability convenience. This is the honest cost of having no ledger |
| Relative trust finds competence rather than seniority, and does not concentrate | **argued** — simulated, and the direction held across every setting tried: standing tracked competence rather than arrival order, did not concentrate, and a late high-quality entrant was not locked out. No rate from those simulations is quoted here, because every parameter in them was chosen by us and the rates are properties of that choice rather than of the protocol. An earlier version of this table quoted a collusion figure that was read against the wrong baseline and understated the effect roughly threefold |
| **Time is verifiable** | **open, and the document previously implied otherwise by omission** — `issued_at` and `observed_at` are strings the signer chooses. Nothing anchors them. A signature proves a key emitted a claim, never when. There is no global clock, deliberately, so any evaluation resting on how old an identity is rests on nothing |
| **Standing decays without fresh evidence** | **open** — nothing in the design reduces standing; only new adverse evidence does, which an actor that stops transacting never produces. Trust is a claim about the present and evidence is a claim about the past, and conflating them lets standing be banked and spent later. **Still open in the live path, and the distinction matters:** the mechanism now exists — depth as elapsed participation, and a decay hook the reference evaluator will use — but its default is not to decay, so an evaluator that leaves it there is choosing to let standing be banked. A mechanism nobody switches on is not a property of the system |
| **Aged identities are distinguishable from organic ones** | **open, and this is the strongest known attack** — the combination of the three rows above. An adversary creates identities at staggered intervals, keeps them unlinked to its principal, has them perform real work with real strangers for real counter-signatures, and defects with all of them at once. Every receipt is genuine. Age cannot be checked. Standing does not lapse. Nothing here detects it, and the countermeasures used against this pattern elsewhere — funding-source analysis, clustering, requiring a cost to exist — are unavailable in a system with no registry, no clock and free identities. **Still open, and detection is not merely missing but refused**: until it defects, such an adversary is doing genuine work at full honest cost and is indistinguishable from the participants this exists for, so anything that could tell them apart would be the surveillance machine this design rejects. What has changed is only where the argument now points. The accrual is not controllable; the *spend* may be, if relying parties hold each extension at or below what the subject would forfeit by betraying it, making a coordinated defection one-shot and break-even at best. That is an accounting argument, **not yet supported by simulation** — no sweep here models a patient defector, and this row must not be softened until one does |
...
| Volume is bounded | **open at the protocol layer, mitigated at the host** — one identity stored 60 valid batches in 0.9 s; a 5,000-record batch was accepted. A Vera host now rate-limits its own ingest, but that is an operator defending its own CPU and database, not admission control on evidence: it excludes nobody, because hosts are plural and substitutable. The protocol still attaches no cost to volume, and the intended answer — pull rather than push, so that unrequested evidence is never retrieved and there is no gate and therefore no gatekeeper — is **not built** |
